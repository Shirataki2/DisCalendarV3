...
pub fn get(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let table_name = table
        .name
        .clone()
        .unwrap_or_else(|| table_ident.to_string().to_ascii_lowercase());
    ...
    let (pk_column_ident, pk_column_type) = match fields_with_pk.len() {
        ...
        1 => (
            fields_with_pk.first().unwrap().ident.as_ref().unwrap(),
            fields_with_pk.first().unwrap().ty.clone(),
        ),
        _ => abort!(
            derive_input.span(),
...
pub fn create(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let field_idents = fields_not_skip
        .iter()
        .map(|f| f.ident.clone().unwrap())
        .collect::<Vec<_>>();
    let field_names = fields_not_skip
        .iter()
        .map(|f| f.ident.clone().unwrap().to_string())
        .collect::<Vec<_>>()
        .join(",");
    let incremental = (1..=fields_not_skip.len())
        .map(|i| format!("${}", i))
        .collect::<Vec<_>>()
        .join(",");
    ...
    let query = format!(
        ...
        table_name, field_names, incremental
    );
...
pub fn update(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let fields_not_skip_nor_pk = fields
        .iter()
        .filter(|f| !f.skip && !f.pk)
        .collect::<Vec<_>>();
